...
bool DataBase::createConnection()  //创建一个连接
{
    ...
    //以后就可以用"combobox_account"与数据库进行连接了

     QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE", "combobox_account");
     if(QSqlDatabase::contains("sqlite1")){
               db = QSqlDatabase::database("combobox_account");
     }else{
               db = QSqlDatabase::addDatabase("QSQLITE", "combobox_account");
     }
...
void DataBase::close()
{
    QSqlDatabase db = QSqlDatabase::database("combobox_account"); //建立数据库连接
    db.close();
}
...
bool DataBase::insert(QString account)//向表comboBox中插入数据(accountd)
{
    QSqlDatabase db = QSqlDatabase::database("combobox_account"); //建立数据库连接
    QSqlQuery query(db);
    ...
    accountList<<account;
    qDebug() << accountList<<account;
    query.bindValue(":account",accountList);
    bool success=query.execBatch();
    if(!success)
    {
        QSqlError lastError = query.lastError();
        qDebug() << lastError.driverText() << QString(QObject::tr("插入失败Combobox"));
        return false;
    }
    return true;
}
...
void DataBase::add_account()
{
    ...
    while(query.next())//一行一行地遍历
    {
        ...
        QString account_exist=query.value("account").toString();
        accounts.push_back(account_exist);
    }
}

bool DataBase::deleteByaccount(QString account_input)  //在表comboBox中删除account所对应的数据
{
    QSqlDatabase db = QSqlDatabase::database("combobox_account"); //建立数据库连接
    QSqlQuery query(db);
...
bool DataBase::query_comboBox(QString account_input)      //根据账号查询在数据库comboBox中是否已存在该账号
{
    QSqlDatabase db = QSqlDatabase::database("combobox_account"); //建立数据库连接
    QSqlQuery query(db);
...
//将元素插入表中
bool DataBase::insert_shortcut(int btnid,int module,QString resource,QString chinese,int whichpage, int index)
{
